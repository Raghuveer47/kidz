...
/**
 * Class 2 — Parts of Computer-1: hardware intro, classification table, input theory, links to device pages.
 */
export default function PartsOfComputerOne({ content, classId }) {
    const rows = content.classificationRows || [];
    const inputDevicesBase = `/class${classId}/input-devices`;
    ...
    const categoryStyle = {
        ...
        'Processing & Internal Components': {
            shell: 'border-amber-300',
            cap: 'from-amber-500 to-orange-500',
            th: 'bg-amber-900 text-amber-50 border-amber-700',
            zebra: 'bg-amber-50/45',
        ...
        <div className="space-y-10 font-nunito">
            <section className="relative overflow-hidden rounded-3xl border-2 border-indigo-300 bg-gradient-to-br from-indigo-100 via-sky-50 to-cyan-100 p-6 md:p-8 shadow-[0_12px_28px_-16px_rgba(79,70,229,0.45)]">
                ...
                <div className="relative">
                    ...
                    <p className="mt-3 whitespace-pre-line text-justify text-base leading-relaxed text-slate-700 md:text-lg">
                        {content.hardwareBody}
                    </p>

                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl border border-indigo-200 bg-white/85 p-3">
                            <p className="text-sm font-bold text-indigo-800">Hardware = things you can touch ✋</p>
                        </div>
                        <div className="rounded-2xl border border-cyan-200 bg-white/85 p-3">
                            <p className="text-sm font-bold text-cyan-800">Software = instructions on screen 💻</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <p className="mb-4 text-slate-600">
                    This table groups common devices the way we list them in your Computer Devices notes: by
                    category, name, and what the device mainly does.
                </p>
                <div className="space-y-7">
                    {Object.entries(rowsByCategory).map(([category, categoryRows]) => (
                        ...
                        >
                            <div className={`flex items-center gap-2 bg-gradient-to-r px-4 py-3 text-white ${categoryStyle[category]?.cap || 'from-slate-600 to-slate-700'}`}>
                            ...
                            <div className="overflow-x-auto bg-white">
                                <table className="w-full min-w-[660px] border-collapse text-left text-sm">
                                    <thead>
                                        <tr>
                                            <th className={`border px-4 py-3 font-extrabold ${categoryStyle[category]?.th || 'bg-slate-900 text-slate-50 border-slate-700'}`}>
                                                Device name
                                            </th>
                                            <th className={`border px-4 py-3 font-extrabold ${categoryStyle[category]?.th || 'bg-slate-900 text-slate-50 border-slate-700'}`}>
                                                What it does
                                    ...
                                    <tbody>
                                        {categoryRows.map((row, idx) => (
                                            <tr
                                                ...
                                                className={idx % 2 === 0 ? 'bg-white' : categoryStyle[category]?.zebra || 'bg-slate-50/60'}
                                            >
            ...
            <section className="grid gap-6 md:grid-cols-2">
                <div className="rounded-2xl border-2 border-emerald-200 bg-emerald-50/60 p-6">
                    <h2 className="text-lg font-extrabold text-emerald-950 md:text-xl">Why we need input devices</h2>
                    <ul className="mt-4 list-disc space-y-2 pl-5 text-slate-800">
                        {(content.whyInputDevices || []).map((line, i) => (
                            <li key={i}>{line}</li>
                        ))}
                    </ul>
                </div>
                <div className="rounded-2xl border-2 border-amber-200 bg-amber-50/60 p-6">
                    <h2 className="text-lg font-extrabold text-amber-950 md:text-xl">How input devices work</h2>
                    <ul className="mt-4 list-disc space-y-2 pl-5 text-slate-800">
                        {(content.howInputDevicesWork || []).map((line, i) => (
                            <li key={i}>{line}</li>
                        ))}
                    </ul>
                </div>
            ...
            <section className="rounded-2xl border-2 border-sky-300 bg-white p-1 shadow-md">
                ...
                >
                    <span className="font-nunito text-base font-extrabold md:text-lg">
                        Input devices — explore each one
                    </span>
                    <ChevronRight className="h-6 w-6 shrink-0" aria-hidden />
                </Link>
                <p className="px-4 pb-4 pt-3 text-sm text-slate-600 md:px-5">
                    Opens the <strong className="text-slate-800">Keyboard</strong> lesson first. Use the sidebar
                    list under Parts of Computer-1 to open any other input device.
                </p>
            </section>
        </div>
    );
