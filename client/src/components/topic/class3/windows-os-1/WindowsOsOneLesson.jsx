...
/**
 * Class 3 — Windows OS-1 sub-lessons: sections as Parts-of-Computer-2–style cards
 * (gradient cap + content) with blue/slate palette—not indigo/emerald.
 */
export default function WindowsOsOneLesson({ content, classId, showBackLink = true }) {
    const sections = content.sections || [];
    const bulletTone =
        'text-justify text-sm leading-relaxed text-slate-700 md:text-base [&_li]:text-justify [&_ul]:marker:text-blue-600';

    return (
        <div className="space-y-10 font-nunito">
            {showBackLink ? (
                <div className="rounded-2xl border-2 border-blue-300 bg-gradient-to-r from-blue-50 to-slate-50 p-4 shadow-sm">
                    <Link
                        to={`/class${classId}/windows-os-1.html`}
                    ...
                    >
                        ...
                        Back to Windows OS-1 overview
                    </Link>
                </div>
            ) : null}

            <section className="relative overflow-hidden rounded-3xl border-2 border-blue-400 bg-gradient-to-br from-slate-100 via-blue-50 to-slate-100 p-6 shadow-[0_12px_28px_-16px_rgba(37,99,235,0.3)] md:p-8">
                ...
                <div className="space-y-6">
                    {sections.map((section, idx) => (
                        <div
                            ...
                            className="overflow-hidden rounded-2xl border-2 border-blue-400 shadow-[0_10px_24px_-12px_rgba(15,23,42,0.22)]"
                        >
                            <div className="flex flex-wrap items-center gap-2 bg-gradient-to-r from-blue-600 to-slate-700 px-4 py-3 text-white">
