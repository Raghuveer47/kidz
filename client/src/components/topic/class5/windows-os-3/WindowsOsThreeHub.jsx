...
/** Class 5 — Windows OS-3 hub: OS installation overview (Windows, Linux, Mac). */
export default function WindowsOsThreeHub({ content, classId }) {
    const osDetail = content.osDetail || {};
    const hubHeroImage = content.hubHeroImage;
    const heroTitle = content.windowsOsSmartBasicsTitle || 'Windows OS-3 — Installing Operating Systems';
    ...
    return (
        <div className="space-y-10 font-nunito">
                    ...
                    <div className="mt-4 overflow-hidden">
                        {hubHeroImage ? (
                            <div className="mx-auto mb-4 w-full max-w-[420px] overflow-hidden rounded-2xl border-2 border-indigo-200 bg-white shadow-inner lg:mb-0 lg:float-left lg:mr-8">
                                <img src={hubHeroImage} alt="" className="h-auto w-full object-cover" loading="lazy" decoding="async" />
                            </div>
                        ) : null}
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl border border-indigo-300 bg-white/90 p-3 shadow-sm">
                            <p className="text-justify text-sm font-bold text-indigo-900">
                                <strong>Installation</strong> puts the operating system onto a computer so programs can run—usually
                                done by adults or IT staff.
                            </p>
                        </div>
                        <div className="rounded-2xl border border-violet-300 bg-white/90 p-3 shadow-sm">
                            <p className="text-justify text-sm font-bold text-slate-800">
                                <strong>Three families</strong>—Windows, Linux, and macOS each have different setup steps and desktop
                                styles.
                            </p>
                        </div>
                    </div>
                    {(osDetail.keyPoints || []).length > 0 ? (
                        <div className="mt-6 grid gap-3 sm:grid-cols-2">
                            {(osDetail.keyPoints || []).map((point) => (
                                <article key={point.title} className="rounded-2xl border border-indigo-200/90 bg-white/95 p-4 shadow-sm">
            ...
            {(content.sections || []).length > 0 ? (
                <section>
                    <h2 className="mb-4 text-xl font-extrabold text-slate-900 md:text-2xl">Installation topics (preview)</h2>
                    <div className="space-y-4">
                        {content.sections.map((section, idx) => (
                            <div key={section.subtitle} className="overflow-hidden rounded-2xl border-2 border-slate-300 bg-white shadow-sm">
                                ...
                                {section.image ? (
                                    <ImageBesideSection
                                        section={{ subtitle: section.subtitle, image: section.image, body: section.body }}
            ...
            <section className="grid gap-6 md:grid-cols-2">
                <div className="rounded-2xl border-2 border-indigo-300 bg-indigo-50/70 p-6">
                    <h2 className="text-lg font-extrabold text-indigo-950 md:text-xl">Why we study OS installation</h2>
            ...
            <section className="rounded-2xl border-2 border-indigo-400 bg-white p-1 shadow-md">
                <Link
                    to={`/class${classId}/windows-os-3-install-windows.html`}
                ...
                >
                    <span className="font-nunito text-base font-extrabold md:text-lg">
                        Start with Installation of Windows OS →
                    </span>
                ...
                <p className="px-4 pb-4 pt-3 text-justify text-sm text-slate-600 md:px-5">
                    Opens the first lesson. Use the sidebar under <strong className="text-slate-800">WINDOWS OS-3</strong> for Linux
                    and Mac installation pages anytime.
                </p>
            </section>
        </div>
    );
